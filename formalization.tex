...
\gap ~|~  r                                                                 & \mbox{silo reference} \\                                
\gap ~|~  \iota                                                             & \mbox{future value} \\
\gap ~|~  h                                                                 & \mbox{host value} \\
p ::=     \texttt{spore}~\{~\seq{x : T = v}~; (x: T) \Rightarrow t~\}       & \mbox{spore value} \\     
\iota  ::= (h,i) \quad \text{where}~i \in \mathbb{N}                        & \mbox{future value} \\
                                                                            & \\   
T ::=                                                                       & \mbox{{\it{types:}}} \\                          
...
\inferrule[\textsc{R-Spawn}] { 
  ...
  \{ (E[\texttt{spawn}(\texttt{spore}~\{~\seq{x : T = v}~; (x: T) \Rightarrow t~\})], Q, S)^h \} \cup H
  \\ \twoheadrightarrow
  \{ (E[h'], Q, S)^h, ((\texttt{spore}~\{~\seq{x : T = v}~; (x: T) \Rightarrow t~\})~\{\}, \epsilon, \epsilon)^{h'} \} \cup H
}

% TODO send spore instead of a value `v`
\inferrule[\textsc{R-Populate}] { 
  S'' = S' + (\iota \mapsto v) \quad \iota = (h', i) \quad i, r~\text{fresh}
} {
  \{ (E[\texttt{populate}(h', v)], Q, S)^h, (t', Q', S')^{h'} \} \cup H
  \twoheadrightarrow
  \{ (E[r], Q, S)^h, (t', Q', S'')^{h'} \} \cup H
}
