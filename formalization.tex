...
materialized silos. In case of a silo reference of the form $Mat(\iota)$
we assume that there is a mapping in the corresponding silo store $S$,
such that $S(\iota) = \text{Some}(v)$ for some value $v$.
...
\inferrule[\textsc{R-Map}] { 
  r' = \text{Mapped}((h, i), r, p, \text{None}) \quad i~\text{fresh}
} { 
  (E[\texttt{map}(r, p)], Q, S)^h \longrightarrow (E[r'], Q, S)^h
}

\inferrule[\textsc{R-FMap}] { 
  r' = \text{FMapped}((h, i), r, p, \text{None}) \quad i~\text{fresh}
} { 
