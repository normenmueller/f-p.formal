...
\newcommand{\Req}[2]{\text{Req}(#1, #2)}
\newcommand{\Fut}[2]{\text{Fut}(#1, #2)}
...
\inferrule[\textsc{R-Await2}] {
  S(\iota) = \text{Some}(\Fwd{r}, P) \quad
...
% What if host(r) != h?
\inferrule[\textsc{R-Await3}] {
  S(\iota) = \text{Some}(\Fwd{r}, P) \quad
  S(id(r)) \neq \text{Some}(\Val{v}, P')
} {
  (E[\texttt{await}(\iota)], Q, S)^h
  \longrightarrow
  (E[\texttt{await}(\iota)], {\Req h r} :: Q, S)^h
  % result: S(id(r)) = Some(Val(v), ..)
}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\inferrule[\textsc{R-Res}] {
  Q  = \text{Res}(\iota, v, P) :: Q'          \quad
...
\inferrule[\textsc{R-Req1Local}] {
  Q         = \text{Req}(h, r) :: Q'               \quad %
  S(id(r))  = \text{Some}(\Fwd{r'}, P)             \quad %
  S(id(r')) = \text{Some}(\Val{v}, P')             \quad %
} {
  (E[\texttt{await}(\iota')], Q, S)^h
  \rightarrow
  (E[\texttt{await}(\iota')], Q' \cdot Res(id(r), v, P), S')^h
}
...
\inferrule[\textsc{R-Req2Local}] {
  ...
  (E[\texttt{await}(\iota')], Q, S)^h
  \rightarrow
  (E[\texttt{await}(\iota')], \text{Req}(h, r') :: Q, S')^h
}
...
\inferrule[\textsc{R-ReqMapLocal}] {
  ...
  P = \emptyset \implies S'' = S' - id(r') \land P \neq \emptyset \implies S'' = S'
} {
  (E[\texttt{await}(\iota')], Q,  S)^h
  \rightarrow
  (E[\texttt{await}(\iota')], Q', S'')^h
...
%\inferrule[\textsc{R-ReqMap2Local}] {
%  Q          = \text{Req}(h, r) :: Q'                     \quad %
%  r          = \text{Mapped}(\iota, r', p, \text{None})   \quad %
%  S(id(r'))  \neq \text{Some}(\Val{v}, P)
%} {
...
\inferrule[\textsc{R-ReqFMapLocal}] {
  ...
  r''        = p(v)                                        \quad %
  host(r'')  = h                                           \quad %
  S' = S + (\iota \mapsto (\Fwd{r''}, \emptyset))
} {
  (E[\texttt{await}(\iota')], Q, S)^h
  \rightarrow
  (E[\texttt{await}(\iota')], Q, S')^h
...
%\inferrule[\textsc{R-ReqFMap2Local}] {
%  Q          = \text{Req}(h, r) :: Q'                      \quad %
%  r          = \text{FMapped}(\iota, r', p, \text{None})   \quad %
%  S(id(r'))  \neq \text{Some}(\Val{v}, P)                  \\
%} {
...
\inferrule[\textsc{R-ReqPersistLocal}] {
  Q          = \text{Req}(h, r) :: Q'                      \quad %
  r          = \text{Persist}(\iota, r', \star)    \quad %
  \iota      = (h, i) \quad %
  S(id(r'))  = \text{Some}(\Val{v}, P)                     \\
  S'         = S  + (\iota \mapsto (\Val{v}, P \star \{h\}))  \quad %
  P = \emptyset \implies S'' = S' - id(r') \land P \neq \emptyset \implies S'' = S'
} {
  (E[\texttt{await}(\iota')], Q,  S)^h
  \rightarrow
...
%\inferrule[\textsc{R-ReqPersist2Local}] {
%  Q          = \text{Req}(h, r) :: Q'                     \quad %
%  r          = \text{Persist}(\iota, r')   \quad %
%  S(id(r'))  \neq \text{Some}(\Val{v}, P)
%} {
%  (E[\texttt{await}(\iota')], Q, S)^h
%  \rightarrow
...
\inferrule[\textsc{R-Req1Map}] { 
  Q          =  \Req {h'} r :: Q'                          \quad
  S(id(r)) \neq \text{Some}(\Val{v}, P)                    \quad
  r          = \text{Mapped}(\iota, r', p, \text{None})    \\
  S(id(r'))  = \text{Some}(\Val{v'}, P')                   \quad
  p(v')      = v''                                         \quad
  S'         = S + (\iota \mapsto (\Val{v''}, \emptyset))  \quad
  m          = \text{Res}(\iota, v'')
  ...
  \{ (R[\texttt{await}(\iota')], Q, S)^h, (t, Q'', S''')^{h'} \} \cup H
  \twoheadrightarrow
  \{ (R[\texttt{await}(\iota')], Q', S'')^h, (t, Q'' \cdot m, S''')^{h'} \} \cup H
}

\inferrule[\textsc{R-Req2Map}] { 
  Q          =  \Req {h'} r :: \hat{Q}                     \quad
  S(id(r)) \neq \text{Some}(\Val{v}, P)                    \quad
  r          = \text{Mapped}(\iota, r', p, \text{None})    \\
  S(id(r')) \neq \text{Some}(\Val{v'}, P')
} { 
  \{ (E[\texttt{await}(\iota')], Q, S)^h, (t', Q', S')^{h'} \} \cup H
  \twoheadrightarrow
  \{ (E[\texttt{await}(\iota')], \Req h {r'} :: Q, S)^h, (t', Q', S')^{h'} \} \cup H
}
...
\inferrule[\textsc{R-Req1FMap}] {
  Q          = \Req {h''} r :: Q''                         \quad
  S(id(r)) \neq \text{Some}(\Val{v}, P)                    \quad
  r          = \text{FMapped}(\iota, r', p, \text{None})   \\
  S(id(r'))  = \text{Some}(\Val{v'}, P')                   \quad
  p(v')      = r''                                         \quad
  S''        = S + (\iota \mapsto (\Fwd{r''}, \emptyset))  \\
  P' = \emptyset \implies S''' = S'' - id(r') \land P' \neq \emptyset \implies S''' = S''
...
\inferrule[\textsc{R-Req1Fwd}] {
  ...
  \{ (E[\texttt{await}(\iota)], Q, S)^h, (t, Q', S')^{h'} \} \cup H
  \twoheadrightarrow
  \{ (E[\texttt{await}(\iota)], Q'', S)^h, (t, Q' \cdot Res(id(r), v, P), S')^{h'} \} \cup H
}
...
\inferrule[\textsc{R-Req2Fwd}] {
  Q         = \text{Req}(h', r) :: \hat{Q}           \quad %
  S(id(r))  = \text{Some}(\Fwd{r'}, P)               \quad %
  S(id(r')) \neq \text{Some}(\Val{v}, P')            \quad %
  S'        = S + (id(r') \mapsto \text{Fut}(h', r)) \quad %
  h''       = host(r')                               \quad %
  m         = \text{Req}(h', r', r)
} {
  \{ (E[\texttt{await}(\iota)], Q, S)^h, (t', Q', S')^{h'}, (t'', Q'', S'')^{h''} \} \cup H
  \twoheadrightarrow
  \{ (E[\texttt{await}(\iota)], \hat{Q}, S)^h, (t', Q', S')^{h'}, (t'', \text{Req}(h, r') :: Q'', S'')^{h''} \} \cup H
}

% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX

\inferrule[\textsc{R-Req4}] { 
  E = \text{Req}(h'', r, \iota'') :: E''                            \quad
  r = \text{FMapped}(\iota, h, \text{Mat}(\iota_s), p, \text{None}) \quad
  S(\iota) = \text{None} \quad S(\iota_s) = \text{Some}(v)          \\
  p(v) = r'                                                         \quad 
  loc(r') = \iota'                                                  \quad
  S(\iota') = \text{Some}(v')                                       \quad
  S'' = S + (\iota \mapsto v')                                      \quad 
  m = \text{Res}(\iota'', v')
} { 
