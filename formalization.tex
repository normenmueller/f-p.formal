...
\inferrule[\textsc{WF-Ref1}] {
  \Delta(\omega) = T  \quad
  \Delta(id(r)) = T' \quad
  \emptyset ; \emptyset \vdash p : T' \Rightarrow T~\{\ldots\} \quad
  \Delta \vdash r
} {
...
\inferrule[\textsc{WF-Ref2}] {
  \Delta(\omega) = T  \quad
  \Delta(id(r)) = T' \quad
  \emptyset ; \emptyset \vdash p : T' \Rightarrow \texttt{SiloRef}[T]~\{\ldots\} \quad
  \Delta \vdash r
} {
...
\inferrule[\textsc{WF-Env2}] {
  ...
  \Delta ; [\iota \mapsto \texttt{SiloRef}[T]]\Sigma \vdash [\iota \mapsto r]\mu
}

\inferrule[\textsc{WF-Env3}] {
  \Delta ; \Sigma \vdash \mu
} {
  \Delta ; [\iota \mapsto \texttt{Future}[T]]\Sigma \vdash [\iota \mapsto \text{Some}(v)]\mu
}
...
% 13.
\item By 4.b) and T-Loc, $\Sigma(\iota) = \texttt{SiloRef}[T'']$.
% 14.
\item By 1.c), 2.c), and WF-Env2
  \begin{enumerate}[label=(\alph*)]
  \item $\Delta(id(r)) = T''$
  \item $\Delta \vdash r$
  \end{enumerate}
% 15.
\item By 1.c), 2.e), 12., and WF-Env2-3, $\Delta' ; \Sigma \vdash \mu$.
% 16.
\item By 2.g), 4.c), and 5., $\Gamma ; \Sigma' \vdash p : (T'' \Rightarrow T' \{~\texttt{type}~\mathcal{C} = \seq{T}~\})$. 
% 17.
\item By 12., 14.a-b), 16., and WF-Ref1, $\Delta' \vdash r'$.
% 18.
\item By 2.f), 5., 12., 15., 17., and WF-Env2, $\Delta' ; \Sigma' \vdash \mu'$.
% 19.
\item 6., 10., 11., 12., and 18. close this case.
\end{enumerate}
