...
\inferrule[\textsc{R-FMap}] { 
  ...
  (E[\texttt{flatMap}(r, p)], Q, S)^h
  \longrightarrow
  (E[r'], Q, S)^h
}

\inferrule[\textsc{R-Persist}] { 
  r' = \text{Persist}((h, i), r) \quad
  i~\text{fresh}
} { 
  (E[\texttt{persist}(r)], Q, S)^h
  \longrightarrow
  (E[r'], Q, S)^h
}
...
\inferrule[\textsc{R-Res}] {
  Q  = \text{Res}(\iota, v) :: Q'          \quad
  S' = S + (\iota \mapsto (v, \emptyset))  
} {
  (E[\texttt{await}(\iota')], Q, S)^h
  \longrightarrow
  (E[\texttt{await}(\iota')], Q', S')^h
...
\inferrule[\textsc{R-Req1Local}] {
  Q         = \text{Req}(h, r) :: Q'               \quad %
  S(id(r))  = \text{Some}(Fwd(r'), P)                 \quad %    XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
  S(id(r')) = \text{Some}(Val(v), P)                    \quad %  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
} {
  (E[\texttt{await}(\iota')], Q, S)^h
  \rightarrow
  (E[\texttt{await}(\iota')], Q' \cdot Res(id(r), v), S')^h
}

\inferrule[\textsc{R-ReqMap1Local}] {
  Q          = \text{Req}(h, r) :: Q'                      \quad %
  r          = \text{Mapped}(\iota, r', p, \text{None})    \quad %
  % we know: S(\iota') = None        
  S(id(r'))  = \text{Some}(v, P)                           \\
  v'         = p(v)                                        \quad %
  S'         = S  + (\iota \mapsto (v', \emptyset))        \quad %
...
\inferrule[\textsc{R-ReqFMap1Local}] {
  Q          = \text{Req}(h, r) :: Q'                      \quad %
...
\inferrule[\textsc{R-Req1}] { 
  Q = \text{Req}(h', r, \iota) :: Q' \quad
  S(id(r)) = \text{Some}(v)          \quad
  m = \text{Res}(\iota, v)
} { 
...
\inferrule[\textsc{R-Req2}] { 
  ...
  r = \text{Mapped}(\iota, h, r', p, \text{None}) \quad
  r' = \text{Mat}(\iota_s)                        \quad
  S(\iota) = \text{None}                          \\
  S(\iota_s) = \text{Some}(v)                     \quad
  p(v) = v'                                       \quad
...
\inferrule[\textsc{R-Req3}] { 
  ...
  S(\iota) = \text{None}                                            \quad
  S(\iota_s) = \text{Some}(v)                                       \\
  \quad p(v) = r'                                                   \quad
  loc(r') = \iota'                                                  \quad
  S(\iota') = \text{None}                                           \quad
  host(r') = h'                                                     \quad
  m = \text{Req}(h, r', \iota')                                     \quad
  E''' = \text{Req}(h'', r', \iota'') :: E''
} { 
  \{ (R[\texttt{await}(\iota_f)], E, S)^h, (t, E', S')^{h'} \} \cup H \twoheadrightarrow
  \{ (R[\texttt{await}(\iota_f)], E''', S)^h, (t, E' \cdot m, S')^{h'} \} \cup H
}
