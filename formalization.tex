...
\inferrule[\textsc{R-Res}] {
  Q = \text{Res}(\iota, v) :: Q' \quad S' = S + (\iota \mapsto (v, \emptyset))  % default: persist only for a single usage
} {
  (E[\texttt{await}(\iota')], Q, S)^h \longrightarrow (E[\texttt{await}(\iota')], Q', S')^h
}

\inferrule[\textsc{R-Req1Local}] {
  Q         = \text{Req}(h, r, \iota) :: Q' \quad %
  S(id(r))  = \text{Some}(v, C)             \quad %
...
\inferrule[\textsc{R-Req2Local}] {
  ...
  S(id(r'))  = \text{Some}(v, C)                          \\
  v'         = p(v)                                       \quad %
  S(id(r'))  = \text{Some}(v, C[h \mapsto C(h) - 1])      \\ %
  S'         = S  + (\iota' \mapsto (v', \{ h \mapsto 1 \}))      \quad %
  S''        = S' + (\iota  \mapsto (v', \{ h \mapsto 1 \}))
} {
  (E[\texttt{await}(\iota'')], Q,  S)^h \rightarrow
  (E[\texttt{await}(\iota'')], Q', S'')^h
}

\inferrule[\textsc{R-Req3Local}] {
  Q          = \text{Req}(h, r, \iota) :: Q'              \quad %
  r          = \text{Mapped}(\iota', r', p, \text{None})  \quad %
  % we know: S(\iota') = None
  S(id(r'))  = \text{None}                                \\
...
\inferrule[\textsc{R-SendLocal}] {
  host(r) = h \quad i~\text{fresh}                  \quad
