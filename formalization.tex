...
\inferrule[\textsc{Proc-ReqMap}] {
  ...
  process(h, {\Req {h'} r \iota}, \sigma) = (\sigma', {\Req {h'} r \iota} ::: \epsilon, h)
}
...
\begin{lem}
\emph{(Weakening of Store Typing)}\label{th:weakening-store-typing}
...
\begin{lem}\emph{(Super)}\label{lem:super}
...
% 9.
\item By 2.d), 3., 4.a), 6., and Lemma~\ref{th:weakening-store-typing}, $\Gamma ; \Sigma' \vdash \texttt{map}(r, p) : \texttt{SiloRef}[T']$.
% 10.
\item By 1.a), 2.d), 6., and Lemma~\ref{th:weakening-store-typing}, $\Gamma ; \Sigma' \vdash t : T$.
% 11.
...
% 8.
\item By 7. and Lemma~\ref{lem:queue-reordering}, $\Sigma \vdash Q \cdot {\Req h r {id(r)}}$.
% 9.
...
% $\Sigma' \vdash \sigma'$
% $\Sigma' \vdash Q'$
% $\Gamma ; \Sigma' \vdash t : T$  // weakening
% goal:
% $\Sigma' \vdash (t, \sigma', Q')^h$
