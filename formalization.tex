...
\begin{defn}[Message processing]
  The function $process \in \mathcal{H} \times m \times S \times \mathcal{H} \rightharpoonup Q \times S$ handles the processing of a message resulting in a new message queue and a new store; here, $S = \mathcal{H} \times \mathbb{N} \rightharpoonup \Val{v} \uplus \Fwd{r}$ is the type of a store (see Def.~\ref{def:store}). $process$ is defined in Fig.~\ref{fig:process}.
\end{defn}
